<?php
	...
	class DbActionTemplate implements iDbActionTemplate
	{
		...
		public function _select ($_params = ['*'], $_whereStr = null, $_limitStr = null, $_orderbyStr = null)
		{
			...
			$sQuery = 'SELECT ' . implode(DELIMITER, $_params) . ' FROM ' . $this -> tableName . (empty($_whereStr) ? null : ' WHERE ' . $_whereStr) . (empty($_limitStr) ? null : ' LIMIT ' . $_limitStr) . (empty($_orderbyStr) ? null : ' ORDER BY ' . $_orderbyStr) . ';';
